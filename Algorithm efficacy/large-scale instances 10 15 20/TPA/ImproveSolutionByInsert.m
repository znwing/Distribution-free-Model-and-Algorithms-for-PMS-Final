        ...
        if k~=s
            for i=1:n
                if x(k,pi(k,i))~=0
                    ...
                    [delta,ComparedMat_changed,x_changed]=InsertOperation(n,i,p,k,s,sigma,ComparedMat,pi,x); %Invoke the insert operation to insert the item at row k, column i into row s, column p
                    if delta<0
                        x=x_changed;
