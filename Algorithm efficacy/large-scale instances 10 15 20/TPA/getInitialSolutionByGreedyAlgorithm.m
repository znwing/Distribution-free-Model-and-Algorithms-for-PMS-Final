...
obj=0;
p=ones(1,m);
ComparedMat=zeros(m,n); %In the case of pi sorting, ComparedMat(k,i) stores the sum of variances in the first i columns of the kth row.
...
while sum(d)~=n
    for k=1:m
        ...
        increment(1,k)=sum_sigma(k)+sigma(k,pi(k,p(k)));               %So we need to directly look at the next one on the kth machine.
    end
